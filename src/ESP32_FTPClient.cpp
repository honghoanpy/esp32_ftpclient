...
void ESP32_FTPClient::GetFTPAnswer (char* result, int offsetStart) {
  ...
  while (!client.available() && millis() < _m + timeout) delay(100);
...
void ESP32_FTPClient::ContentListWithListCommand(const char * dir, String * list) {
  ...
  unsigned long _m = millis();
  while( !dclient.available() && millis() < _m + timeout) delay(100);
...
/*fw filename format: md5_fwname.bin. Ex: aabfd69acbed74f28d501812a6da420f_firmwarerole_v1.0.17.bin */
void ESP32_FTPClient::DownloadFirmware(const char * filename, size_t size, const char * md5 ) {
  FTPdbgn("Send RETR");
  if(!isConnected()) return;
  client.print(F("RETR "));
  client.println(F(filename));
  ...
  Update.begin();       // start with max available size
  if(!Update.setMD5(_md5)) FTPdbgn("MD5 error.. ");;   // Set MD5 to checksum FW
  FTPdbgn("Loading FW ... ");
  while(dclient.available())
  {
    if( length >= 512)
    ...
    else
    {
      ...
      if(Update.write(_buf, length) == length){
        //FTPdbgn(".");
      }
      length = 0;
    }
  ...
  if(Update.end(true)) FTPdbgn("Ended"); //true to set the size to the current progress
  else FTPdbgn("Fail");
}
